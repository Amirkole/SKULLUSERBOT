...
async def yt_search(skull):
    try:
        skull = urllib.parse.quote(skull)
        html = urllib.request.urlopen(
            "https://www.youtube.com/results?search_query=" + skull
        )
...
async def extract_time(skull, time_val):
    if any(time_val.endswith(unit) for unit in ("m", "h", "d", "w")):
        unit = time_val[-1]
        time_num = time_val[:-1]  # type: str
        if not time_num.isdigit():
            skull.edit("Invalid time amount specified.")
            return ""
        if unit == "m":
...
async def trumptweet(text):
    ...
    mayank = r.get("message")
    skullurl = url(mayank)
    if not skullurl:
        return "check syntax once more"
    with open("temp.png", "wb") as f:
        f.write(requests.get(mayank).content)
...
async def waifutxt(text, chat_id, reply_to_id, bot, borg):
    ...
    skull = await sticcers[0].click("me", hide_via=True)
    if skull:
        await bot.send_file(int(chat_id), skull, reply_to=reply_to_id)
        await skull.delete()
