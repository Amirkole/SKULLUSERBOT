...
async def gablist(event):
    ...
    if len(gbanned_users) > 0:
        for a_user in gbanned_users:
            if a_user.reason:
                GBANNED_LIST += f"馃憠 [{a_user.chat_id}](tg://user?id={a_user.chat_id}) for {a_user.reason}\n"
            else:
                GBANNED_LIST += (
                    f"馃憠 [{a_user.chat_id}](tg://user?id={a_user.chat_id}) Reason None\n"
                )
    else:
...
CMD_HELP.update(
    {
        ...
        \n\n  鈥�  **Syntax : **`.gban <username/reply/userid> <reason (optional)>`\
\n  鈥�  **Function : **__Bans the person in all groups where you are admin .__\
\n\n  鈥�  **Syntax : **`.ungban <username/reply/userid>`\
\n  鈥�  **Function : **__Reply someone's message with .ungban to remove them from the gbanned list.__\
\n\n  鈥�  **Syntax : **`.listgban`\
\n  鈥�  **Function : **__Shows you the gbanned list and reason for their gban.__\
\n\n  鈥�  **Syntax : **`.gmute <username/reply> <reason (optional)>`\
\n  鈥�  **Function : **__Mutes the person in all groups you have in common with them.__\
\n\n  鈥�  **Syntax : **`.ungmute <username/reply>`\
\n  鈥�  **Function : **__Reply someone's message with .ungmute to remove them from the gmuted list.__"
    }
)
