...
DEFAULTUSERBIO = str(DEFAULT_BIO) if DEFAULT_BIO else " 釛棭釓嗎帰釓嗎憥釓� 釓炨弳釓︶棿 釒⑨弳釛搬棿  "
CHANGE_TIME = Config.CHANGE_TIME
DEFAULTUSER = str(AUTONAME) if AUTONAME else "skull"
...
async def main(event):
    ...
    downloader = SmartDL(skull, downloaded_file_name, progress_bar=False)
    downloader.start(blocking=False)
    if DIGITALPICSTART:
...
CMD_HELP.update(
  ...
  鈥�  **Function : **__Rotating image along with the time on it with given angle if no angle is given then doesnt rotate. You need to set __`DOWNLOAD_PFP_URL_CLOCK`__ in heroku__

  鈥�  **Syntax : **`.digitalpfp`
  鈥�  **Function : **__Your profile pic changes to digitaltime profile picutre__

  鈥�  **Syntax : **`.bloom`
  鈥�  **Function : **__Random colour profile pics will be set along with time on it. You need to set__ `DOWNLOAD_PFP_URL_CLOCK`__ in heroku__
