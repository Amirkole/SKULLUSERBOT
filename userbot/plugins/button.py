...
async def _(event):
    if event.fwd_from:
        return
    reply_to_id = None
    skullinput = "".join(event.text.split(maxsplit=1)[1:])
    if event.reply_to_msg_id:
        reply_to_id = event.reply_to_msg_id
    await event.get_reply_message()
    # soon will try to add media support
    if not skullinput:
        skullinput = (await event.get_reply_message()).text
    if not skullinput:
        await edit_or_reply(event, "`Give me something to write in bot inline`")
        return
    skullinput = "Inline buttons " + skullinput
    tgbotusername = Config.TG_BOT_USER_NAME_BF_HER
    results = await bot.inline_query(tgbotusername, skullinput)
