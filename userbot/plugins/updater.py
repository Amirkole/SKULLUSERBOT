...
async def upstream(event):
    ...
    if changelog == "" and not force_update:
        await event.edit(
            "\n`SKULLUSERBOT is`  **up-to-date**  `with`  "
...
@bot.on(admin_cmd(outgoing=True, pattern=r"badskull$"))
@bot.on(sudo_cmd(pattern="badskull$", allow_sudo=True))
async def upstream(event):
    event = await edit_or_reply(event, "`Pulling the bad skull repo wait a sec ....`")
    off_repo = "https://github.com/hackelite01/SKULLUSERBOT.git"
    skullcmd = f"rm -rf .git"
    try:
...
CMD_HELP.update(
    {
        ...
        "\n\n  •  **Syntax : **`.badskull`"
        "\n  •  **Function :** Shifts from official skull repo to skull's repo(for gali commands)"
    }
)
